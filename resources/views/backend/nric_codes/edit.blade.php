...
@section ('title', trans('labels.backend.nric_codes.management') . ' | ' . trans('labels.backend.nric_codes.edit'))

@section('page-header')
    <h1>
        {{ trans('labels.backend.nric_codes.management') }}
        <small>{{ trans('labels.backend.nric_codes.edit') }}</small>
    </h1>
@endsection

@section('content')
    {{ Form::model($nric_code, ['route' => ['admin.nric_codes.update', $nric_code], 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'PATCH']) }}

        <div class="box box-success">
            <div class="box-header with-border">
                <h3 class="box-title">{{ trans('labels.backend.nric_codes.edit') }}</h3>
